                ...
                <div className="bg-neutral-50 dark:bg-neutral-900/50 border-b border-neutral-200 dark:border-white/5 py-16 sm:py-24">
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
                        ...
                        <h1 className="text-4xl font-bold tracking-tight text-neutral-900 dark:text-white sm:text-5xl mb-6">
                            {t('techStacksPage.title', { defaultValue: 'Recommended Tech Stacks' })}
                        </h1>
                        <p className="text-lg text-neutral-600 dark:text-neutral-400 max-w-2xl mx-auto mb-10">
                            {t('techStacksPage.subtitle', { defaultValue: 'Don\'t know where to start? We\'ve curated the best modern technology combinations for beginners and pros alike.' })}
                        </p>
                        ...
                        <div className="max-w-xl mx-auto space-y-4">
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    placeholder={t('techStacksPage.searchPlaceholder', { defaultValue: "Search stacks (e.g., 'SaaS', 'React', 'Enterprise')..." })}
                                    value={searchQuery}
                            ...
                            <div className="flex justify-center pt-2">
                                ...
                                >
                                    ...
                                    {t('techStacksPage.createStack', { defaultValue: 'Create Your Own Stack' })}
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
                    ...
                    <div className="flex flex-col gap-6 mb-12">
                        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-3">
                                {categories.map((c) => {
                                    const cat = c as StackCategory | 'All';
                                    let catLabel: string = cat;
                                    if (cat === 'All') catLabel = t('common.all', { defaultValue: 'All' });
                                    else if (cat === 'AI') catLabel = t('common.ai', { defaultValue: 'AI' });
                                    else catLabel = t(`common.${cat.toLowerCase()}`, { defaultValue: cat });
                            ...
                            <div className="relative z-20">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-colors ring-1 ${isToolFilterOpen || selectedTools.length > 0
                                        ...
                                        : 'bg-white border border-neutral-200 text-neutral-600 dark:bg-white/5 dark:border-transparent dark:text-neutral-400 dark:ring-white/10 hover:bg-neutral-50 dark:hover:bg-white/10'
                                        }`}
                                >
                                    <Filter className="w-4 h-4" />
                                    {t('techStacksPage.filterByTools', { defaultValue: 'Filter by Tools' })}
                                ...
                                <AnimatePresence>
                                    {isToolFilterOpen && (
                                        <motion.div
                                            ...
                                            className="absolute right-0 mt-2 w-72 md:w-96 max-h-96 overflow-y-auto rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 shadow-2xl p-4 ring-1 ring-black/5 z-50 rtl:right-auto rtl:left-0"
                                        >
                                            <div className="flex justify-between items-center mb-3">
                                                <span className="text-sm font-medium text-neutral-900 dark:text-white">{t('techStacksPage.selectTools', { defaultValue: 'Select required tools' })}</span>
                                                <button
                                                    onClick={() => setSelectedTools([])}
                                                    className="text-xs text-neutral-500 hover:text-neutral-900 dark:hover:text-white"
                                                >
                                                    {t('techStacksPage.clearAll', { defaultValue: 'Clear all' })}
                                                </button>
                                            </div>
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`px-3 py-1.5 rounded-md text-xs font-medium border transition-all ${isSelected
                                                                ...
                                                                : 'bg-neutral-50 border-neutral-200 text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 dark:hover:border-neutral-500 dark:hover:text-white'
                                                                }`}
                                                        >
                                ...
                                {selectedTools.map(slug => {
                                    ...
                                    return (
                                        <span key={slug} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs">
                                            {tool ? t(`tools.${tool.slug}.name`, { defaultValue: tool.name }) : slug}
                    ...
                    {filteredStacks.length === 0 ? (
                        <div className="text-center py-20">
                            ...
                            <h3 className="text-xl font-semibold text-neutral-900 dark:text-white mb-2">{t('techStacksPage.noStacksFound', { defaultValue: 'No matching stacks found' })}</h3>
                            <p className="text-neutral-400">{t('toolsPage.tryAdjusting', { defaultValue: 'Try adjusting your filters or search terms.' })}</p>
                            <button
                            ...
                            >
                                {t('techStacksPage.clearAll', { defaultValue: 'Clear all filters' })}
                            </button>
                        </div>
                    ) : (
                        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-2">
                            {filteredStacks.map((stack) => (
                                <motion.div
                                    ...
                                    layout
                                    className="group relative bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 rounded-2xl overflow-hidden hover:shadow-lg dark:hover:bg-white/10 transition-all duration-300 hover:border-indigo-500/50 flex flex-col"
                                >
                                    <div className="absolute top-4 right-4 rtl:right-auto rtl:left-4">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${stack.category === 'Web' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                                            ...
                                            {stack.category === 'AI' ? t('common.ai', { defaultValue: 'AI' }) : t(`common.${stack.category.toLowerCase()}`, { defaultValue: stack.category })}
                                        </span>
                                    </div>

                                    <div className="p-8 flex-1 flex flex-col">
                                        <h3 className="text-2xl font-bold text-neutral-900 dark:text-white mb-3 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors pr-20 rtl:pr-0 rtl:pl-20">
                                        ...
                                        <div className="space-y-3 mb-8 bg-neutral-50 dark:bg-black/20 p-4 rounded-xl border border-neutral-100 dark:border-transparent">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <span className="text-sm text-neutral-600 dark:text-neutral-300 line-clamp-1">
                                                    {t(`techStacks.${stack.slug}.whenToUse.0`, { defaultValue: stack.whenToUse[0] })}
                                                </span>
                                            </div>
                                            <div className="flex items-start gap-3">
                                                <XCircle className="w-5 h-5 text-red-500/80 flex-shrink-0 mt-0.5" />
                                                <span className="text-sm text-neutral-600 dark:text-neutral-300 line-clamp-1">
                                                    {t(`techStacks.${stack.slug}.whenNotToUse.0`, { defaultValue: stack.whenNotToUse[0] })}
                                                </span>
                                            </div>
                                        </div>

                                        <div className="flex flex-col gap-6 mt-auto sm:flex-row sm:items-end sm:justify-between">
                                            <div className="flex -space-x-2 overflow-visible flex-wrap gap-y-2 pl-2 rtl:pl-0 rtl:pr-2">
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <motion.div
                                                                ...
                                                                className="h-10 w-10 rounded-full ring-2 ring-white dark:ring-neutral-900 bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-xs font-bold text-neutral-600 dark:text-neutral-300 uppercase relative cursor-pointer hover:ring-indigo-500 hover:scale-105 hover:bg-white dark:hover:bg-neutral-700 transition-all shadow-md"
                                                                title={t(`tools.${tool.slug}.name`, { defaultValue: tool.name })}
                                                            >
                                                ...
                                                {stack.tools.length > 6 && (
                                                    <div className="h-10 w-10 rounded-full ring-2 ring-white dark:ring-neutral-900 bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-xs font-bold text-neutral-500 dark:text-neutral-400">
                                                        +{stack.tools.length - 6}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center justify-center rounded-full bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 px-6 py-2.5 text-sm font-semibold transition-transform active:scale-95 hover:bg-neutral-800 dark:hover:bg-neutral-200 whitespace-nowrap"
                                            >
                                                {t('techStacksPage.viewGuide', { defaultValue: 'View Guide' })}
